...
class RSS extends XMLWriter {
	 ...
	 */
	function __construct($title, $link, $description, $num, $fulltitle) {
		$this->openMemory();
    	...
    	// validator's requirement
    		...
    		$this->writeElement('title', $fulltitle);
	 ...
	 */
	function addItem($title, $link, $description, $thumbnail, $pubDate) {
		// avoid null item
		...
		$this->startElement("item");
			...
			if ($pubDate != null) {
				$this->writeElement('pubDate', $pubDate);
			}
		$this->endElement();
	}
