...
var videoElement;
var navOffset = -35;
var isanIpad = navigator.userAgent.match(/iPad/i) != null;
...
var VideoJS = Class.extend({
  ...
  init: function (element, setOptions) {
    ...
    this.options = {
      ...
      defaultVolume: 1, // Will be overridden by localStorage volume if available
    ...
    //Listen for clicks on Share
    this.shareControl.addEventListener("click", this.onShareControlClick.context(this), false);
    ...
    if (this.poster) this.poster.addEventListener("mouseover", this.onVideoMouseOver.context(this), false);
  ...
  buildController: function () {
    ...
    <li class="vjs-base-progress"></li>
    <li class="vjs-load-progress"></li>
    <li class="vjs-play-progress"></li>
    <li class="vjs-play-scrubber"></li>
    </ul>
    </li>
    ...
    <li class="vjs-fullscreen-control"></li>
    <li class="vjs-share-control">
    <span class="share-text">Share</span><span class="share-icon"></span>
    </li>
    <li class="vjs-share-links">
    <div id="share-links">
    <a/><a/><a/><a/>
    </div>
    </li>
    ...
    */
    ...
    this.controls.style.bottom = 0 + "px";
    ...
    // Create the base progress display
    this.baseProgress = _V_.createElement("li", { className: "vjs-base-progress" });
    this.progressHolder.appendChild(this.baseProgress)
    ...
    // Create the playing progress display
    this.playScrubber = _V_.createElement("li", { className: "vjs-play-scrubber" });
    this.progressHolder.appendChild(this.playScrubber);

    $('.vjs-play-scrubber').html('<span class="scrubOverlay"></span>')
    ...
    this.volumeControl = _V_.createElement("li", {
      ...
      innerHTML: "<ul><li id='1'></li><li id='2'></li><li id='3'></li><li id='4'></li><li id='5'></li><li id='6'></li></ul>"
    });
    this.controls.appendChild(this.volumeControl);
    this.volumeDisplay = this.volumeControl.children[0];

    // Crete the fullscreen control
    this.fullscreenControl = _V_.createElement("li", { className: "vjs-fullscreen-control", innerHTML: "" });
    this.controls.appendChild(this.fullscreenControl);

    // Create the share control
    this.shareControl = _V_.createElement("li", { className: "vjs-share-control" });
    this.controls.appendChild(this.shareControl);
    this.shareText = _V_.createElement("span", { className: "share-text", innerHTML: "Share" });
    this.shareControl.appendChild(this.shareText);
    this.shareIcon = _V_.createElement("span", { className: "share-icon", innerHTML: "" });
    this.shareControl.appendChild(this.shareIcon);

    // Create Share Links Control
    var html = "<div id='share-links'>"
                  + "<a class='socialshare fb'><img alt='Facebook Share' src='/images/btn-facebook.png' /></a>"
    //+ "<a class='addthis_button_facebook'><img src='/images/btn-facebook.png' /></a>"
                  + "<a class='addthis_button_google_plusone_share'><img src='/images/btn-gplus.png' /></a>"
                  + "<a class='addthis_button_twitter'><img src='/images/btn-twitter.png' /></a>"
                  + "<a class='addthis_button_email'><img src='/images/btn-email.png' /></a>"
                + "<span></span></div>";
    this.shareLinks = _V_.createElement("li", { className: "vjs-share-links", innerHTML: html });
    this.shareLinks.style.display = "none";
    this.controls.appendChild(this.shareLinks);

    //iPads don't read the source properly in dynamically loaded videos.
    //If this is an iPad, reload the source and start playing.

    if (isanIpad) {

      $(window).bind('orientationchange', this.onChangeOrientation.context(this), false);

      $('video').src = $('video source:first').attr('src');
      $('video').load();
      $('.UIBlock').height($(document).height());
      scroll(0, 0);
      this.video.play();
    }
  ...
  hideController: function () {
    if ($('video').parents('.tileVideo').length == 0) {
      $('.vjs-controls').stop().animate({ 'bottom': navOffset }, 250)
      this.shareLinks.style.display = "none";
    }
  },
  ...
  onPlay: function (event) {
    ...
    //console.log(event);
    //update unice url                
    //var vdoBaseURI = event.srcElement.baseURI;
    var vdoBaseURI = window.location.href
    var vdoName = event.target.currentSrc;

    var urlparts = vdoBaseURI.split('?');
    if (urlparts.length >= 2) {
      var prefix = encodeURIComponent("video") + '=';
      var pars = urlparts[1].split(/[&;]/g);
      for (var i = pars.length; i-- > 0; )
        if (pars[i].lastIndexOf(prefix, 0) !== -1)
          pars.splice(i, 1);
      vdoBaseURI = urlparts[0] + '?' + pars.join('&');
    }

    var $vdo = $(event.target);
    var vdoDesc = ($vdo.attr('title')) ? $vdo.attr('title') : '';
    var vdoTitle = ($vdo.attr('data-desc')) ? $vdo.attr('data-desc') : '';
    //console.log($vdo.attr('data-desc'));      

    var videoid = $vdo.parent().find('input._videoid').val();

    vdoName = vdoName.split('/').pop().split('.')[0];
    //vdoBaseURI += "&video=" + vdoName;
    vdoBaseURI += "&video=" + videoid;
    this.updateShareLinks(vdoBaseURI, vdoDesc, vdoTitle); // re-update link
  ...
  onProgressHolderMouseDown: function (event) {
    ...
    if ((event).preventDefault && !isanIpad) (event).preventDefault();
  ...
  // Adjust the volume when the user clicks or drags on the volume control
  ...
  onShareControlClick: function (event) {
    if (this.shareLinks.style.display == "none")
      this.shareLinks.style.display = "block";
    else
      this.shareLinks.style.display = "none";
  },
  ...
  onVideoMouseOver: function (event) {
    if ($('video').parents('.tileVideo').length == 0) {
      $('.vjs-controls').stop().animate({ 'bottom': '0' }, 250);
    }
  },
  ...
  updatePlayProgress: function () {
    ...
    var theTotalWidth = (_V_.getComputedStyleValue(this.progressHolder, "width").replace("px", ""))
    var theWidth = ((this.video.currentTime / this.video.duration) * theTotalWidth);
    var margin = 0;
    var scrubberPos = (((theTotalWidth - (2 * margin)) / theTotalWidth) * theWidth) + margin;

    this.playProgress.style.width = theWidth + "px";
    this.playScrubber.style.left = scrubberPos + "px";
  ...
  setPlayProgressWithEvent: function (event) {
    ...
    var theActivePos = event.touches ? event.touches[0].pageX : event.pageX;
    var newProgress = _V_.getRelativePosition(theActivePos, this.progressHolder);
  ...
  setVolumeWithEvent: function (event) {
    ...
    var volNum = parseFloat(event.target.id) / 6;
    this.setVolume(volNum);
  },

  // Update the volume control display
  // Unique to these default controls. Uses borders to create the look of bars.
  updateVolumeDisplay: function () {
    var volNum = Math.round(this.video.volume * 6);

    $('.vjs-volume-control ul').children().each(function (k, v) {
      if (k < volNum)
        $(v).addClass("vjs-volume-level-on");
      else
        $(v).removeClass("vjs-volume-level-on");
    });
  },


  updateShareLinks: function (vdoShareURL, title, desc) {
    ...
    addthis.toolbox('#share-links',
                    ...
                    {
                        url: ((vdoShareURL) ? vdoShareURL : 'http://www.bk.com'),
                        title: ((title) ? title : ''),
                        description: ((desc) ? desc : ''),
                        templates: {
                        twitter: ((title) ? title + " {{url}} #Burgerking" : 'Browse the BK menu, find your local BK restaurant, or learn about career opportunities! {{url}}')
                        }
                    });

    socialshare.toolbox('#share-links', {
      url: ((vdoShareURL) ? vdoShareURL : 'http://www.bk.com'),
      title: ((title) ? title : ''),
      description: ((desc) ? desc : '')
    });
