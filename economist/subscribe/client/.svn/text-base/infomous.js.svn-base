...
for (var d in defaults){
	if(typeof(vars[d]) == 'undefined') vars[d] = defaults[d];
}

var $i = jQuery.noConflict();

var INFOMOUS_TRACKER = 'infomousTracker';
var _gaq = _gaq || [];

var $target = $i('#' + vars.target);

/*if(vars.width.indexOf('%') != -1 || vars.height.indexOf('%') == -1){ //size is set in percent
	//give the container an explicit size
	$i('.infomous-container').css({
		'width': vars.width,
		'height': vars.height
	});
	//assign to $target and then read value in px
	$target.css({
		'width': vars.width,
		'height': vars.height
	});
...
//final size assignment, always in px
$target.css({
	'width': vars.width, 
	'height': vars.height,
	'position':'relative'
...
function infomousJS(){
	if(vars.trackGoogle) loadGA();
	trackEvent(document.URL, 'jload', 'jviewer');
...
function onConfigLoad(config){
	...
	if(typeof(infomous_load) == 'function') infomous_load('js');
	
	if(vars.loadAtStart) makeRequest();
...
function makeRequest(){
	...
	var reqVars = ['type', 'width', 'height', 'maxWords', 'linkageThreshold', 'popularWordCutoff', 'textOption', 'maxResults', 'minChars', 'dict', 'focused', 'hidden', 'blocked', 'ngrams', 'hiddenSource', 'ngramsSource', 'fromDaysBack', 'toDaysBack', 'adsPercent', 'zoom', 'fontScale', 'groups'];
	/*for (var rp in reqVars)	{
		var value = vars[reqVars[rp]];
		if (typeof(value) != 'undefined' && value !== ""){
			req += '&'+ reqVars[rp] +'='+ encodeURIComponent(value);
		}
	}*/
	for(var v in vars){
		if(reqVars.indexOf(v) != -1 && typeof(vars[v]) != 'undefined' && vars[v] != ""){
...
function render(data){
  ...
  for (var w in words) {
    ...
    var $text = trim(words[w].text);
    if(typeof(words[w].br) != 'undefined' && parseInt(words[w].br) > 0){
    	var br = parseInt(words[w].br);
    	$text = $text.substring(0, br) +'<br/>'+ $text.substring(br + 1);
    }
    ...
    $word.click(function(e){
    	if(focused.indexOf($i(this).html()) != -1){
    		vars.focused = '';
    	}else{
    		vars.focused = $i(this).html();
    		trackEvent(document.URL, 'jfocus', vars.focused);
    	}
        makeRequest();
    	e.stopPropagation();
    });
    $word.mouseover(function(e){
        trackEvent(document.URL, 'jmouseover', $i(this).text());
    });
    $new.append($word);
    ...
    $hide.click(function(e){
    	$i(this).parent().hide();
    	var hword = $i(this).siblings('.word').html();
    	if('hidden' in vars){
    		vars.hidden += ','+ hword;
    	}else{
    		vars.hidden = hword;
        ...
        makeRequest();
    	e.stopPropagation();
    	trackEvent(document.URL, 'jhide', hword);
    });
    $new.append($hide);
	    ...
	    for(var ai in a_ids){
	    	for(var a in ads){
	    		if(ads[a].id == a_ids[ai]){
	    			$list.append('<li class="i-ad"></li>');
	    			var $ad = $list.find('.i-ad');
	    			if (ads[a].img) {
	    				$ad.append('<div class="i-ad-img" style="float:left;margin-right:5px"><img src="'+ads[a].img+'"></div>');	
	    			}
	    			$ad.append('<div class="i-ad-txt"><a class="line1" href="'+ ads[a].url +'" target="infomous-target">'+ ads[a].line1 +'</a><br/><span class="line2">'+ ads[a].line2 +'</span></div>');
	    ...
	    for(var si in s_ids){
	      for(var s in sources){
	        if(sources[s].id == s_ids[si]){
	          $list.append('<li class="i-source"><a class="line1" href="'+ sources[s].url +'" target="infomous-target">'+ sources[s].title +'</a><br/><span class="line2">'+ sources[s].caption +'</span></li>');
	        }
	      }
	    }
    }
    ...
    $new.css({
      'left': (pos[words[w].id].x - pos[words[w].id].ow/2) + 'px',
      'top': (pos[words[w].id].y - pos[words[w].id].oh/2) + 'px'
    });
    ...
    $new.tooltip({
    	...
    	relative: false,
    	offset:[0, -pos[words[w].id].ow],
    	effect:"fade", delay:300,
    	onBeforeShow: function() {
    		this.getTrigger().children(".hide").show();
    		 trackEvent(document.URL, 'jsources_view', this.getTrigger().children("span").html());
    	},
...
//Sources & ads follow
  $i('.i-source a').click(function() {
	trackEvent(document.URL, 'jsource_follow', $i(this).attr('href'));
  });
  $i('.i-ad a').click(function() {
	trackEvent(document.URL, 'jad_follow', $i(this).attr('href'));
  });
...
/** Tracking functions **/
var gatName =  'infomousTracker';

function loadGA(){
  // google analytics
  _gaq.push([INFOMOUS_TRACKER + '._setAccount', vars.trackGAAccount]); // create GA tracker named INFOMOUS_TRACKER
...
function trackEvent(url, action, label) {
  if(vars.trackGoogle) {
	  _gaq.push([INFOMOUS_TRACKER + '._trackEvent', url, action, label]);
  }

  if(vars.trackOmniture) {
    var omEvent = 'infomous/' + action + '/' + label;
    var omUrl = url + '/' + omEvent;
    if(omEvent.indexOf('?') == -1) omEvent += '?p1=infomous';
    else omEvent += '&p1=infomous';
    var omObject = {"href" : omUrl};
    if(typeof(s) != 'undefined') s.tl(omObject, 'o', omEvent);
  }
}
