...
var CloudToolbar = function(config) {
	...
	var self = this,
	    $toolbar = $('<div/>', { 'class': 'cloud-toolbar' });
	...
	config.has = function(widget) {
		return this.controls === undefined
			|| this.controls.toLowerCase().indexOf(widget.toLowerCase()) != -1;
	};
	...
	this.add_to_dom = function($target) {
		...
		$toolbar.css( 'width', css_size( config.width || 'auto' ) );

        var $w, $widgets = $toolbar.find('.ctrls-widget');
        ...
        //function _hide(obj) {
            ...
            //for (key in obj) {
                ...
                //if (wgt.jquery !== undefined) {
                    //if (wgt.position().top >= t_height)
                        //wgt.hide();
                    //else
                        //wgt.show();
                //}
                //else if (
                    //Object.prototype.toString.call(wgt) === "[object Object]")
                    //_hide(wgt);
            //}
        //}
	...
	function add_word(word, id) {
		...
		_update_dispatch_wordfilter.call(self, id, word_str, true);
	}
