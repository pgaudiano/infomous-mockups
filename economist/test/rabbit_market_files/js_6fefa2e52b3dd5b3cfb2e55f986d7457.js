...
Drupal.theme.prototype.policyBox = function (caller) {
  ...
  policy += '<p>You are solely responsible for all content you post to the site. Libel, copyright and trade mark infringement, links to commercial websites, products, or sales materials, and offensive or threatening language are not permitted and may be removed based on our comments policy (for more information, please review our <a href="/legal/terms-of-use">terms of use</a>).</p>';
...
Drupal.behaviors.formFeatures = function(){
  ...
  var validateFields = ('input[required="required"], input.required, select.required');
...
function submitEditable(editableForm, storedData) {
  $('input#edit-submit', editableForm).live('click', function(e){
    ...
    var submitBtn = $('<input type="submit" class="form-submit  editable-submit" value="Save changes" id="edit-submit" name="op">');
...
Drupal.behaviors.masthead = function(context) {
  ...
  $('a.show-login').click(function(e) {
    ...
    // scroll down to the login form instead.
      $('html, body').animate({
        scrollTop: ($('form#user-login').offset().top - 30)
      }, 600);
...
;
Drupal.behaviors.hpAudioVideo = function(context) {
  // Flash parameters if different from the default ones.
  var flashParams = Drupal.settings.flash_params;
  // The element that gets replaced by the flash object.
  var elementClass = Drupal.settings.element_class;
  // Version check based upon the values entered above in "Globals".
  var hasReqestedVersion = DetectFlashVer(8, 0, 0);
  // Check to see if the version meets the requirements for playback.
  if (hasReqestedVersion) {
    if (elementClass) {
      $('.' + elementClass).flash(flashParams);
    }
    $(document.documentElement).removeClass('no-flash');
  }
  else {  // flash is too old or we can't detect the plugin.
    var alternateContent = '<div><a href="http://www.adobe.com/go/getflash/" class="missing-flash">You must have the <span>Adobe Flash Player</span> '
                         + 'installed on your computer to view this interactive content.</a></div>'
    // Check for iphone/ipod/ipad devices
    if((navigator.userAgent.match(/iP(hone|od|ad)/i))) {
      alternateContent = '<div class="noflash-box">This interactive content is not available. Your device doesn’t support Flash Player.</div>'
    }
    $("body object, body embed", context).each(function(){
      var $this = $(this);
      // Replace the flash object with an instructinal box.
      $this.replaceWith(alternateContent);
    });
  }
}
;
