...
if (typeof s_account == "undefined") {
  ...
  var s_account = "economistcomprod";
...
// Only enable the filters below when they use the same 3rd party cookie configuration

s.linkInternalFilters = "javascript:,economist.com,economistsubscriptions.com";
...
function s_doPlugins(s) {
	...
	/* Get props from query string and page values */

	setArticleDetails(s);



	/* Set timeparting values */
	...
	s.prop19 = s.getQueryParam('sa_campaign');

	s.prop20 = s.getQueryParam('source');

	s.prop21 = s.getQueryParam('quizname');

	s.prop22 = s.getQueryParam('fsrc');



	//setContentCategory(s); // Currently data population not confirmed

	setTechVendor(s);

	setClassifiedValues(s);
	...
	// get the subdomain of the referrer

	s.prop14 = s.eVar14 = getReferringSubDomain();





	// get the sort query param from article comment pages

	var sortValue = s.getQueryParam("sort");
...
function setArticleDetails(s) {

	var scStoryID = s.getQueryParam('story_id');

	if (scStoryID) s.prop6 = scStoryID;

	if (isEmpty(s.prop5) && typeof getTitle == 'function') s.prop5 = getTitle();
...
function setClassifiedValues(s) {

	/* Handle Classifieds Tag Array */

	var classifiedId = s.getQueryParam('sitd');
	...
	try{
		...
		if (typeof (NPdata) != "undefined" && isArray(NPdata)) {

			for (var j = 0; j < NPdata.length; j++) {

				var nvpNPData = NPdata[j].split("=");



				switch (nvpNPData[0]) {

					case "CLASSIFIED_SECTION": s.prop16 = Trim(nvpNPData[1]); break;

					case "CLASSIFIED_ORGANISATION": s.prop17 = Trim(nvpNPData[1]); break;
	...
	if (classifiedId.length > 0) s.prop17 = [s.prop17, classifiedId].join("|")

}



function setContentCategory(s) {

	if (!isEmpty(s.prop1)) s.prop24 = s.prop1 + "|" + s.prop2;

}



function setTechVendor(s) {

	if (!isEmpty(s.prop25)) return;



	// Test for Cold Fusion extension

	if (window.location.href.match(/.cfm/gi)) {
	...
	if (document.location.host.match(/economistsubscriptions.com/gi)) {
	...
	if (document.location.host.match(/worldpay/gi)) {
...
function seteVars(s) {
	...
	for (var i = 1; i <= 42; i++) {
		...
		var scQuery = "if (s.prop" + i + " && !s.eVar" + i + ") s.eVar" + i + " = s.prop" + i;
...
function getReferringSubDomain() {



	//Pull in referrer from browser

	var rURL = document.referrer;



	//Do this if referrer exists

	if (rURL != "" && rURL != null){



		//Start at end of protocol

		if(rURL.indexOf("//") != -1){

			var refStart = rURL.indexOf("//")+2;
		...
		//End where the directory structure begins

		if (rURL.indexOf("/",refStart) != -1){

			var dirStart = rURL.indexOf("/",refStart)
			...
			var dirStart = rURL.length;

		}



		//Pull the substring between these points

		var rSubDomain = rURL.substring(refStart,dirStart);
	...
	//Make an array of internal filters

	var internalFilters = s.linkInternalFilters.split(",");



	//Cycle through the array to check for an internal referring domain
	...
	for (i=0;i<=internalFilters.length;i++){
		...
		if (rSubDomain.indexOf(internalFilters[i]) != -1) {
...
s.visitorNamespace = "economist";

s.trackingServer = "stats.economist.com";

s.trackingServerSecure = "sstats.economist.com";
