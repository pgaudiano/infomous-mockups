...
function onTemplateReady(e) {
  ...
  if (multimedia_channel != undefined) {
    var tabs = tabBar.getData();
    for (var i=0, len=tabs.length; i<len; i++) {
      if (multimedia_channel == tabs[i].displayName.toLowerCase().replace(/[^a-z]/g, '')) {
        tabBar.setSelectedIndex(i);
      }
    }
  }
  playlistId = (tabBar) ? tabBar.getSelectedData().id : video.getCurrentVideo().lineupId;
  // Update the link so it will deep link in correctly.
  updateLink(video.getCurrentVideo().id, playlistId);
}
...
Drupal.behaviors.ec_ajax_form = function(context) {
  $('[data-form-ajax]', context).each(function(){
    ...
    $this.bind('click', function(e){
      ...
      $.getJSON($this.attr('data-form-ajax'), function(data) {
        // Append the form.
        var form = '<div class="ajax-form">' + data.form + '</div>';
        // Remove the ajax loader.
        $(".ajax-loading").remove();
        // Use the custom callback.
        var jsCallback = data.js_callback;
        if (jsCallback) {
...
Econ.validateForm = function(submitBtn) {
  ...
  this.error = function(msg){return '<span class="error-msg">' + msg + '</span>'};
...
Econ.validateForm.prototype.textareaMaxlength = function(field, maxlength){
  ...
  var errorSelector = this.errorSelector;
...
Econ.validateForm.prototype.emptyForm = function(field){
  ...
  return submitBtn.click(function() {
    ...
    if (field.length) {
      var elmLength = $.trim(field.val()).length;
      if (elmLength > 0) {
        $(errorSelector, fieldParent).remove();
      } else {
        if (!$(errorSelector, fieldParent).length){field.before(errorMsg)};
        return false;
      }
    }
    return true;
...
Drupal.behaviors.renderGallery = function(context) {
  var $this, imgHeight, galleryHeight, selected, item, totalItems;
  var gallery = $('.ec-gallery');
  var elmToScroll = 'elm-to-scroll';
  var swipeSize = 30;

  // Loop through the items to create the image list.
  gallery.each(function(){
    $this = $(this);
    
    // Set the height for the entire section; This to allow different image 
    // height.
    imgHeight = $('img', $this).get(0);
    galleryHeight = $(imgHeight).height();
    $this.css('height', galleryHeight + 90);

    // Number of items in the list to set the pager
    totalItems = $('li', $this).size();
    item = 1;
    $('.page', $this).append('<span>' + item + '</span> / ' + totalItems);

    // Add classes to the items to enable the scroll transition.
    var elm = $('li', $this);
    selected = elm.get(0);
    $(selected).addClass(elmToScroll).next().addClass(elmToScroll + '-next');

    // Display the controls when the gallery is loaded.
    $('.ec-gallery-controls').addClass('controls-enabled');

    // Display a swipe message if you are NOT on touch based devices.
    if (!window.Touch) {
      $('.arrows-nav', $this).addClass('arrows-nav-enabled');
      arrowBehavior($this);
    }
    else {
      // We want to show the message just once.
      if (typeof localStorage != 'undefined') {
        var firstAccess = localStorage.getItem('gallery');
        if (!firstAccess) {
          gallery.prepend('<div class="swipe-info"></div>');
          $('.' + elmToScroll + ', .ec-gallery').bind('touchstart', function(){$('.swipe-info').remove();});
        }
  ...
  // Prepare all the variables that are used to determine how to swipe articles.
  function prepareSwipe($this){
    var hswipeStart = 0;
    var hswipeEnd = 0;
    var vswipeStart = 0;
    var vswipeEnd = 0;
    var fingerNr;
    var elm = $('.' + elmToScroll, $this);
    // MOUSE BASED DEVICES.
    // Add the 'click and move forward' functionality.
    if (!window.Touch) {
      elm.bind('click', function(e){
        e.preventDefault();
        if(elm.next().hasClass(elmToScroll + '-next')) {
          slide(elm, 'next');
        }
      });
    }
    // TOUCH BASED DEVICES.
    // Grab x & y coords of the starting touch.
    elm.bind('touchstart', function(e){
      e.preventDefault();
      // Check the number of fingers touching the screen.
      fingerNr = e.originalEvent.touches.length;
      // Perform the swipe if only one finger is touching the screen.
      if (fingerNr == 1) {
        hswipeStart = e.originalEvent.touches[0].pageX;
        vswipeStart = e.originalEvent.touches[0].pageY;
      }
      else {
        return false;
      }
    ...
    // Grab x & y coords of the ending touch.
    elm.bind('touchmove', function(e){
      hswipeEnd = e.originalEvent.touches[0].pageX;
      vswipeEnd = e.originalEvent.touches[0].pageY;
    });
    // Perfom the swipe once calculated the direction.
    elm.bind('touchend', function(e){
      var elm = $(this);
      swipe($this, elm, hswipeStart, hswipeEnd, vswipeStart, vswipeEnd);
      e.stopPropagation();
    });

  }

  // Slide works with css transitions. We assign classes to the visible elements
  // and its closest siblings to create the swipe animation.
  function slide($this, elm, dir) {
    var elmDirR, elmDirL, elmDirLL;
    var classPrev = (dir == 'next') ? elmToScroll + '-prev' : elmToScroll + '-next';
    var classNext = (dir == 'prev') ? elmToScroll + '-prev' : elmToScroll + '-next';
    var pagSel = (dir == 'next') ?  +1 : -1;
    $('.page span', $this).text(parseInt($('.page span', $this).text()) + pagSel);
    if (dir == 'next') {
      elmDirR = elm.next();
      elmDirL = elm.prev();
      elmDirLL = elm.next().next();
    }
    else {
      elmDirR = elm.prev();
      elmDirL = elm.next();
      elmDirLL = elm.prev().prev();
    }
    // The swipe is performed adding classes to 3 list elements. The css classes
    // are set to move the list item according to their position using a CSS left
    // trasformation
    elmDirL
      .removeClass(classPrev);

    elm
    .removeClass(elmToScroll)
    .addClass(classPrev);

    elmDirR
      .removeClass(classNext)
      .addClass(elmToScroll);
    elmDirLL
      .addClass(classNext);

    if ($('.' + elmToScroll + '-prev', $this).length) {
      $('.ec-gallery-prev', $this).removeClass('off');
    }
    else {
      $('.ec-gallery-prev', $this).addClass('off');
    }
    if ($('.' + elmToScroll + '-next', $this).length) {
      $('.ec-gallery-next', $this).removeClass('off');
    }
    else {
      $('.ec-gallery-next', $this).addClass('off');
    }
    prepareSwipe($this);

  }
  // Change the arrow state according to the pager.
  function arrowBehavior($this){
    $('.ec-gallery-prev', $this).addClass('off');
    $('.ec-gallery-prev, .ec-gallery-next', $this).bind('click', function(e){
      var elm = $(this);
      var dir = (elm.hasClass('ec-gallery-prev')) ? 'prev' : 'next';
      if ($('.' + elmToScroll + '-' + dir, $this).length) {
        slide($this, $('.' + elmToScroll, $this), dir);
      }
    });
  }

  // Determine the animation direction.
  function swipe($this, elm, hswipeStart, hswipeEnd, vswipeStart, vswipeEnd) {
    if (elm.next().hasClass(elmToScroll + '-next') && (hswipeStart - hswipeEnd) > swipeSize && Math.abs(vswipeStart - vswipeEnd) < 35) {
      slide($this, elm, 'next');
    }
    if (elm.prev().hasClass(elmToScroll + '-prev') && (hswipeEnd - hswipeStart) > swipeSize && Math.abs(vswipeStart - vswipeEnd) < 35) {
      slide($this, elm, 'prev');
    }
  }
};
...
Drupal.behaviors.userLoad = function(context) {
  ...
  if (Econ.user.user) {
    ...
    // Grab user for logged out user.
      ...
      Econ.user.user = {"uid":0,"name":null,"country":{"id":false,"name":"Unknown","iso":"","region":false}};
      
      // If we are on a page that has a regionalized cover, we need real data for that.
      if (typeof Drupal.settings.regional_covers != 'undefined') {
        // POST to get through Varnish.
        $.post(Drupal.settings.basePath + 'geoip.php', {'1' : '1'}, function(data, textStatus, jqXHR) {
          if (data.status) {
...
Econ.FormFeatures.Validate = function(formName, validateFields) {
  ...
  this.emptyFields = function() {
    ...
    $(validateFields, formName).each(function(){
      ...
      // Pen name is optional.
      if ((($.trim($this.val()) == '' || $this.val() == 'invalid')) && (fieldId != 'edit-name')) {
...
Drupal.behaviors.ecComments = function(context) {
  var commentForm = $('#comment-form', context);
  var submitBtn = $('#edit-submit', commentForm);
  var commentsArea = $('#comments-area');
  var fields = $('#edit-comment', context);
  if (fields.length) {
    var validateCommentForm = new Econ.validateForm(submitBtn);
    validateCommentForm = validateCommentForm.textareaMaxlength(fields, 5000).emptyForm(fields);
  }
...
Drupal.behaviors.ecModeration = function(context) {
  var thisForm = $('#ec-moderation-report-abuse-form', context);
  var submitBtn = $('#edit-submit', thisForm);
  var fields = $('#edit-ec-abuse-explanation', context);

  // Econ.validateForm object can be found into the ec_base.js file.
  if (fields.length) {
    var checkMaxLength = new Econ.validateForm(submitBtn);
    checkMaxLength = checkMaxLength.textareaMaxlength(fields,500);
  }
  thisForm.submit(function(e) {
    ...
    if (checkMaxLength) {
      $.ajax({
        ...
        beforeSend: function(){
          submitBtn.attr('disabled', 'disabled').after('<span class="ajax-working"></span>');
        },
        success: function(){
          $this.parent().remove();
          $('a[data-form-ajax="' + $this.attr("action") + '"]').replaceWith('<span class="reported secondary-action">Reported</span>');
        }
      });
    }
  });
  $('span.remove-form', thisForm).click(function(){
    $('.ajax-form').remove();
    $('a').parent().removeClass('opened');
  });
}

// Callback function to customize the report abuse form.
function reportAbuseAjaxCallback(elm, form) {
  var attachedForm = elm.parents('ul');
  $('.ajax-form').remove();
  attachedForm.append(form);
  $('a').parent().removeClass('opened');
...
Drupal.behaviors.masthead = function(context) {
  ...
  if (document.location.protocol == 'http:') {
    $('a.show-login', context).click(function(e) {
      e.preventDefault();
      var $this = $(this)
      if ($('div#inline-login-box').length) {
        $('div#inline-login-box').remove();
      }
      ...
      // scroll down to the login form instead.
        ...
        $this.after('<div id="inline-login-box"></div>')
        $this.append('<span class="ajax-loader"></span>');
        $('#inline-login-box').load('/ecouser/login/ajax', function() {
          var destination = Drupal.settings.loginform.action;
          $('#inline-login-box').prepend('<span></span><div class="close">Close</div>');
          $('form#user-login').attr('action', destination);
          $('div.close').click(function() {
            $('div#inline-login-box').remove();
          });
          $('div#inline-login-box > span').addClass('login-indicator');
          Drupal.attachBehaviors('#user-login')
...
Drupal.behaviors.hpAudioVideo = function(context) {
  ...
  if (hasReqestedVersion) {
    if (elementClass) {
      $('.' + elementClass, context).flash(flashParams);
    }
...
;
Drupal.behaviors.ecChannelLatestBlogs = function(context){
  $('.latest-blog-posts .created', context).ec_timeago();
};
;
/**
 * @file
